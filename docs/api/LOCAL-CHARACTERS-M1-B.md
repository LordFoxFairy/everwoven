...
状态：2026-09-12实施中；以PROGRESS中的实际测试和浏览器验收为准。正式角色库复用此契约，不新建“数据库角色”页面。
...
原角色库采用编辑副本+确认DTO；提交A后继续编辑B，A成功只更新确认基线及正式ID/revision，B保留dirty。会话连接位于页面导航上方；正式未连接不改用localStorage。保存锁与图片处理锁分离后组合。角色图片上传/读取由M1-C完成前，不接受本地演练图片为正式头像。

本页并不宣称原角色UI、图片上传、剧本版本绑定、真实生成已经验收。各项以PROGRESS记录的证据为准。
