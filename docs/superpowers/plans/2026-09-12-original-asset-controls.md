...
## 接受标准（2026-09-12 本地验收）

- [x] 原角色页选择真实图片→上传完成→保存portraitAssetId→清浏览器业务缓存→服务进程重启→同图片读回。
- [x] 更换与清除头像不影响原图片和其他角色；角色软删除/恢复后引用一致。
- [x] 非法/过大/损坏图片、401/404/413/409/超时/断网有可操作反馈，不报假成功。
- [x] begin和complete分别模拟提交成功丢响应，同原命令重放后1个意图/1个资产，无重复创建。
- [x] 快速点击/卸载/切换角色/连接失效/跨dataset旧响应不会覆盖新表单。
- [x] 演练零正式网络写入；正式禁浏览器存储后依然成功。
- [x] 原剧本聚合在独立用例接通后才启用正式图片槽保存；未保存引用不进入生成。
- [x] 主仓全量、typecheck、生产Chrome及规格/质量复核通过，更新PROGRESS后继续聚合/单入口验收。
...
最终主仓1340/1340、双端类型、生产五Chrome、独立规格/质量PASS；详见implementation/M1-C2记录。勾选“聚合独立接通后启用”表示本片维持禁用边界，不表示聚合已实现。原“当前代码”段保留作实施前缺口记录。
