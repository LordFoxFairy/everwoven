# M1 · 原剧本聚合接线（冻结语义，源码已实现、最终验收中）

依据Dewey实读16表/现root用例的审查，直接替换storyDrafts六操作，不加同义REST、不保留根CRUD兼容层、不另建编辑器。本文件保存实施前冻结的可执行语义；当前源码及验证见[实施记录](../../implementation/M1-D-STORY-AGGREGATE-2026-09-13.md)，不以计划勾选代替生产/CI/主入口验收。
