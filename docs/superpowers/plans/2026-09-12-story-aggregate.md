...
- assetSlots若出现就是完整cover/opening/character三槽对象，null清除；整个字段未出现才保留。移除角色时character强制空，cover/opening不变。
...
- 本次聚合请求/详情使用schemaVersion:1单字面量及datasetId，所有写命令必填。没有旧结构联合或缺字段补齐；缺/错协议标识明确CLIENT_RELOAD_REQUIRED，非法当前结构用固定参数错误。同步全部调用者/fixtures，不留隐藏旧入口。
