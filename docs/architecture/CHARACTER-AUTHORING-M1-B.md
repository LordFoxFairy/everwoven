...
2026-09-12渲染验证：临时隔离安装Mermaid 12.0.0（不改项目依赖），真实Chrome分别渲染依赖图1552×170和时序图1552×1681，两个SVG均有有效尺寸/SQLite标签，主会话复看PNG无重叠截断。网络请求全部阻断，未把文档上传到外部渲染站点。此证据针对该版本渲染器，不承诺不同GitHub渲染器版本完全一致。
