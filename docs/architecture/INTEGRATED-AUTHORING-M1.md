...
| 数据 | Prisma/SQLite15表、零FK、批准的迁移门禁 | 作用域/图片元数据/上传意图及重置流程 |
...
目前代码仍批准旧M0 baseline、15表、0FK、0触发器。正式实施时新baseline替换旧迁移链，schema gate只批准新schema，预计包含上传意图共16业务表。门禁继续校验表/列/索引/迁移checksum/0FK/0触发器；不允许“任何库都打开”。
