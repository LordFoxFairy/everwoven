...
| 层 | 已有且需复用 | 本批缺口 |
|---|---|---|
| 前端 | 原角色库、原Editor完整表单、StoryController、原StoryLibrary及图片端口已接；临时面板已删除 | 已验证；新增冷浏览器门禁见PROGRESS |
| HTTP | 受保护六操作聚合协议、characters、assets及二进制路由、共享本机会话 | 产品源码CI已验证；未发新tag镜像 |
| 应用 | 固定角色版本、完整聚合事务/历史回执、资产生命周期、owner+dataset、WriteGate及CAS | 已通过规格与质量复审 |
