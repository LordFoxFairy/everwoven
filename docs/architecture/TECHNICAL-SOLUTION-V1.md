...
> 2026-09-14 原应用游玩接线更新：见 [PLAY-FLOW-V1](../api/PLAY-FLOW-V1.md)，包含已实现架构/时序、报价接受、回应草稿与恢复契约。已通过本地替身链路验证；已补服务端计时播放会话与原子存档；真实供应商验收、结算和历史分支仍未完成。
...
| 数据与身份 | Prisma、26表、17业务唯一、零FK；本机会话、写事务、revision和命令回执 | 用户库增量升级保留旧数据 |
| 分幕游玩 | 费用确认→执行/查询→私有视频→完整播放→情境建议/自由回应→下一幕 | 本地供应商替身链已验证；付费两幕未验收 |
| 播放与持久化 | 服务端计时会话、顺序回执、不可变已播存档、回应草稿与重启恢复 | 非真人观看证明；历史列表/fork未实现 |
