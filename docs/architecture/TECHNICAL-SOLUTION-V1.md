...
> 2026-09-14 原应用游玩接线更新：见 [PLAY-FLOW-V1](../api/PLAY-FLOW-V1.md)，包含已实现架构/时序、报价接受、回应草稿与恢复契约。已通过本地替身链路验证；已补服务端计时播放会话与原子存档；基础历史分支已接入，按证据结算已接入；真实供应商验收与最终账单对账尚未完成。
...
| 数据与身份 | Prisma、29表、19业务唯一、零FK；本机会话、写事务、revision和命令回执 | 用户库增量升级保留旧数据 |
...
| Provider | 供应商/连接/model分层，OpenRouter文本、MiniMax官方视频与Pollo内部平台适配 | Pollo测试网关鉴权、计费口径与真实视频验收待完成，不混用Key |
| 成本 | 显式报价、阶段费用证据、整回合结算、超额停发及原舞台费用读取 | 文本按账户扣费、视频按封存费率核算；最终账单对账尚未验收 |
