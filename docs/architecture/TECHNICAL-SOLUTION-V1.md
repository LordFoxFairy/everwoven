...
> **2026-09-14 主链实施：** [分段生成内部契约与时序](../api/GENERATION-RUNTIME-V1.md)补齐报价、原子接受、可恢复执行、播放后节点和下一幕来源关系。尚未接通正式 executor 和原页面，不代表产品闭环完成。
...
| 范围 | 当前证据 | 状态 |
|---|---|---|
| 同一3100应用、晴空蓝紫界面、前端Mock演练 | 默认个人入口、分段状态、草稿恢复走查 | 已有原型，不是模型生成 |
| M0-A/B内部Prisma/SQLite | 15表/13真实唯一/零FK；WriteGate、剧本根CRUD、回执/CAS；226项历史测试通过 | 内部用例已验证，未接业务HTTP/UI |
| tRPC | 只有非敏感只读video.configuration | 尚无正式受保护业务写入 |
| 宿主身份/会话、单实例、数据保护、备份 | 有设计 | M0-C后续门槛 |
| 正式人物/素材/经历/视频/事实/分支 | 基础适配或设计，不具完整链路 | M1–M3/B1待实施与实测 |
| 二期AI创作工作台 | [提案](../product/AI-CREATIVE-WORKSPACE-V2-PROPOSAL.md) | 不进入当前一期热路径 |

本轮只更新技术文档和关系/时序图，不修改运行代码、SQL迁移、已有用户数据或付费调用。
