...
打开 `http://127.0.0.1:3100/` 后自动连接本机创作空间，不需要连接码输入框或在终端取码。连接失败显示轻量重试，原表单内容保留；重新连接不自动重放待确认业务命令。

浏览器先只读查询已有会话，未连接时向同源发送 `POST {mode:"local"}`。专用本地启动器与来源检查通过后，宿主内部建立 HttpOnly / SameSite=Strict 会话（8小时），JSON仅返回身份状态和dataset，不返回原始凭据。合法已有会话复用，不在每次刷新新建。自动接入要求浏览器Fetch Metadata为same-origin，普通Web/Docker不启用；本地文档禁止被iframe嵌入。

CLI `runtime:host connect` 只用于需要显式凭据的维护/HTTP工具，不再是产品创作步骤。该模式信任本机OS用户与本应用同源脚本，不宣称隔离同OS用户恶意程序；不是公网用户登录系统。
...
专用启动器接收SIGINT/SIGTERM后不再接受新连接，给在途请求5秒收尾，再关闭本进程持有的长连接（含开发HMR）。Next清理仍不返回时，在10秒截止以失败状态退出；重复信号使用同一停机过程。客户端断线不能证明正在保存的命令未执行，原unknown命令仍按回执恢复，不自动重放。此机制不是数据库备份，也不主动删除开发锁、数据库或其他服务进程。自动验收要求正常exit0；强杀兜底视为失败。
