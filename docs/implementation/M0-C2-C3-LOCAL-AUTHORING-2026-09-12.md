...
- 本机 Docker daemon 返回 500，因此转由 GitHub Linux CI 验收。**[CI 34703786441](https://github.com/LordFoxFairy/everwoven/actions/runs/34703786441) 全部通过**（受测提交 `6e4de7f`）：Docker 构建、demo/dev/prod × 3100/3200 端口、容器非安全 HTTP 浏览器回归、本机 CRUD/进程重启/丢失响应恢复。该次是分支验证，publish/anonymous-pull 按配置跳过；没有发布新镜像或修改 v0.1.0 标签。
