...
**当前交付：原3100/T3应用已接通剧本/角色/图片→固定开局→费用确认→执行与私有视频→播完建议/自由回应→草稿恢复。基础故事足迹、合格存档另开路线、共用预算和重启找回原分支已实现并通过真实Host/SQLite/原页面本地媒体验收。当前代码28表/18业务唯一/零FK。未完成项只保留：真实供应商配置/付费两幕验收，以及实际账单结算；不声称整个V1已完成。**
...
| 后端 | Host/Provider/Worker/私有视频、报价接受、播放会话、不可变存档与独立分支；28表/18业务唯一/零FK | 实际账单结算，Pollo公开接入资料 |
| 产品闭环 | 本地替身两幕链与原Host自然播放/重启验收通过 | 预算授权后的真实两幕及实际结算 |
