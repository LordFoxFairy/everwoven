...
**当前交付：原3100/T3应用已接通剧本/角色/图片→固定开局→费用确认→执行与私有视频→播完后建议/自由回应→草稿恢复。新增服务端计时播放会话和原子已播存档，弱网回执及退出重播修复。SQLite+HTTP+原页面的本地媒体验收通过，真实付费模型验收尚未进行。剩余明确为供应商公开接入信息/配置、实际结算、历史浏览与fork；不把数据库存档节点等同完整分支功能。当前轮最终测试/构建/提交证据见文末。**
...
| 后端 | Host/Provider/Worker/私有视频、报价接受、播放会话及原子存档；26表/17业务唯一/零FK | 实际账单结算，Pollo公开接入资料 |
| 产品闭环 | 本地替身两幕链与原Host自然播放/重启验收通过 | 预算授权后的真实两幕，以及历史浏览/fork |
| 技术方案 | PLAY-FLOW-V1包含现行API、架构/时序、恢复；唯一键登记已同步 | 不用早期方案状态当作现状 |
| 推进 | 本轮收尾现有播放链，不增加二期能力 | 提交现分支并核验同一commit的CI |
