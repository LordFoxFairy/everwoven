...
更新：2026-09-14。**这是实施真值台账，不用设计完成代替功能完成，不用commit代替验收。**

**当前交付：原3100/T3应用已接通剧本/角色/图片→固定开局→费用确认→执行与私有视频→播完建议/自由回应→草稿恢复。基础故事足迹、合格存档另开路线、共用预算和重启找回原分支已实现并通过真实Host/SQLite/原页面本地媒体验收。当前代码29表/19业务唯一/零FK。按证据结算与原舞台费用查看已实现；未完成项为真实供应商配置/付费两幕验收及最终账单对账；不声称整个V1已完成。**
...
2026-09-14 播放与回应收尾：按用户要求收住范围，没有扩展fork或二期功能。新增限时播放会话，核验原私有文件并绑定owner/dataset/storeEpoch/经历修订/turn/media hash；按序记录连续覆盖及会话总时间，裸ended不推进。completePlayback同事务写不可变StateSnapshot/played_segment Savepoint、viewed/awaiting、decision、空回应草稿和原命令回执；失败全部回滚。快照保存固定剧本/profile与已确认场景前缀及父关系。未实现历史浏览/fork；不声称完成完整分支。

独立只读审查Aquinas提出并修复：退出重进复用旧进度、正常网络抖动被相邻到达间隔误拒绝、进度已落库但响应丢失后sequence冲突。现在关闭舞台清播放会话、使用总会话时间约束、保留原progress命令并重放后提交末尾进度。delay/lost receipt和1.9s/2.1s到达回归通过；最终静态复核无本范围阻塞项。移除已确认前缀的任意512幕上限，避免生成完成后因无产品依据的计数门槛拒绝存档；前缀仍按hash验证，历史优化留给后续明确需求。

修正上一commit 85174ab的CI34830520227结论：确实失败在4项原有真实子进程测试的5000ms超时，而非浏览器文案。CI固定2worker；多Node进程测试单独声明启动时间预算，业务断言和运行时期限不变。另同步准备页/返回焦点的浏览器断言，新增正式原Host播放+草稿重启的零模型smoke进入CI。

用户本机已升级第五个增量迁移：原23张表所有旧列逐行摘要、4条migration历史保持；备份在私有目录，位置记于/tmp/everwoven-playback-upgrade.json。新增三张空表，integrity_check=ok、零FK；未清空/seed用户数据。重启前确认generation.json不存在，turn/outbox为0，原3100同应用温和重启。新session2899，证据/tmp/everwoven-closeout-original3100.log。Chrome只读原页面/自动连接/角色剧本/MyPlay/认证generation GET/刷新复用通过（*-final-original-browser.log）。

独立生产构建及真实Host浏览器验收通过：本地5秒MP4自然播放→服务器确认→原子存档→片尾建议→自由回应SQLite保存→真实进程重启后原稿和快照仍在（/tmp/everwoven-closeout-final-build.log、*-final-playback-browser.log）。仅一次性测试Host使用显式fixture，生产应用没有加入Mock或付费测试入口。所有供应商调用为0；Pollo协议/model及预算授权仍未确认，尚无真实两幕/实际账单验收，总goal不标完成。


最终主仓验证：`pnpm test --maxWorkers=2`共138文件2154项通过，runtime/Web源码及测试typecheck通过，独立生产build通过。证据/tmp/everwoven-closeout-final-tests.log、*-final-types.log、*-final-build.log。原3100浏览器两幕演练也通过（*-stage-fixture.log），它只在浏览器测试上下文使用HTTP/媒体替身，不写用户业务表。原Studio完整剧本/人物/图片/不确定回执/重启smoke通过（*-story-browser.log）。

## 2026-09-14 基础分支收尾（本轮）

- 已实现 history.list/get/fork/recover/resume，与原Host/tRPC/舞台同入口。只回看已播事实；原路线到C后从B分叉，child只继承A/B，首个新片段D连接本地fork_base，禁止读取源C。
- 原回应先保存，再创建child；新节点/建议/空草稿独立，固定原剧本/profile/绑定并共享根预算，零任务/预留。确认继续后才响应与报价。
- 原舞台唯一video，足迹为可收纳浮层；预览不写播放证明。fork回执丢失后只读找回，原路线被删除也保留入口。
- 每条前缀最多1000点；完整导演上下文在报价/接受、资金预留前检查65536字符上限。未知责任和同scope进行中的付费阶段挡住新派发，读结果/媒体可恢复。
- 主仓最终验证：140文件、2170测试通过，typecheck通过；隔离生产构建通过。真实Chrome + 原页面/Host/SQLite/MP4烟测覆盖自然播放→输入草稿→回看→fork提交后丢回执→删除source→进程重启→GET找回原child→显式resume。断言只一个video、只一个child、一个预算scope、零child turn、零quote/accept。全部零模型调用。
- 独立只读复核Aquinas关闭来源失效时恢复入口P2。最初草稿失败是烟测误查setup草稿，最终按原Savepoint.interactionEventId精确验证decision草稿，未把脚本问题当成代码修复成果。
- 已加入上述浏览器流程到GitHub CI。API方法、数据字典、架构/时序和真实唯一登记已同步。
- 原用户库第六迁移已应用：私有SQLite backup、26原表旧列数据摘要/5条历史保留、两张新表空、完整性通过、零FK。启动前generation配置不存在、turn/outbox均为0，未触发付费任务。原3100已重启；真实会话下generation.get、history.list、history.recover及原创作入口只读验证通过。未写入演示业务数据，未发模型请求。提交/CI证据另记。

### 剩余验收边界

实际结算仍未实现：当前保存报价上限、预留和文字用量观察，未知责任保留，不以视频已完成冒充最终账单。真实供应商两幕也未验收；用户要求完成实现前不做昂贵测试。当前正式适配为OpenRouter文本/官方MiniMax视频；Pollo完整公开协议与模型身份待提供，不混用账号和Key。二期AI创作画布、实时流/物体点击不纳入收尾。

## 2026-09-14 费用结算闭环（当前轮）

- 前一批f0eea5192edc2c955fc30c302c2302e4f73266cb的[CI34836241289](https://github.com/LordFoxFairy/everwoven/actions/runs/34836241289)已终态success，本轮未扩展二期功能。
- 新增不可变StageCostEvidence，文本保存OpenRouter账户扣费原始decimal；视频保存任务原引用和终态用量，与后续SceneResult独立。缺值不等于0，视频按完整封存费率核算而非声称已有最终发票。
- 三阶段经济证据齐全后与终态同事务整回合结算，释放多余预留；相同证据重复和重启均不重复记账，源路线删除不抹除费用。缺失仍held；已知部分计量也进入超限检查；超限保留真实责任并阻止scope新接受/付费派发。
- 崩溃租约恢复仍保留业务unknown，但完整持久费用证据可在同一事务结算；未重发供应商。
- 原舞台“费用”浮层只读展示确认上限、已结算、仍预留与阶段依据。无需新供应商配置来读取历史费用。无第二网站/视频，无GET结算副作用。
- 主仓最终141文件/2187测试通过，typecheck通过。独立Aquinas复核关闭3项P2：部分已知计量漏算下界、validator超限仍ready、JSON金额精度。实际生产浏览器费用/重启及分支/源删除恢复均通过，最终修订生产构建与费用烟测再次通过。
- 第七迁移已用于原用户库：私有backup、28原表旧列摘要与6条历史全部保持、StageCostEvidence空、完整性OK、零FK。启动前generation.json仍不存在、turn/outbox为0，未启动付费任务。原3100最终重启并通过generation/history/cost的真实会话只读检查、原创作入口与刷新恢复检查，零业务seed和模型请求。当前提交CI另记。
- [费用API/事务/时序](api/GENERATION-SETTLEMENT-V1.md)、[实施计划](superpowers/plans/2026-09-14-generation-settlement.md)、schema/SQL及真实唯一登记同步。新总数29表、19业务唯一。
