import {miniMaxModelLimits, miniMaxRatios} from './minimax-constraints.js';
...
const ratios:readonly string[]=miniMaxRatios;
// This URL-only implementation intentionally stays well below the vendor's 64 MB body cap.
export const MINIMAX_FRAME_URL_MAX_BYTES=8192;
export const MINIMAX_REQUEST_MAX_BYTES=65536;
...
export function buildMiniMaxRequest(input:MiniMaxRequestInput):MiniMaxRequest{
 ...
 const limits=miniMaxModelLimits[input.model];
 if(!(limits.resolutions as readonly string[]).includes(input.resolution))throw Error('所选模型不支持这个分辨率');
 if(!Number.isInteger(input.duration)||input.duration<limits.minSeconds||input.duration>limits.maxSeconds)throw Error('所选模型不支持这个片段时长');
 ...
 if(input.frames!==undefined){
  ...
  for(const key of ['first','last'] as const){
   ...
   if(typeof value!=='string'||value.length>MINIMAX_FRAME_URL_MAX_BYTES||
    encoder.encode(value).byteLength>MINIMAX_FRAME_URL_MAX_BYTES||!value.trim()||value!==value.trim())throw Error('图片地址格式或大小无效');
 ...
 const request={model:input.model,content,resolution:input.resolution,duration:input.duration,ratio};
 if(encoder.encode(JSON.stringify(request)).byteLength>MINIMAX_REQUEST_MAX_BYTES)throw Error('视频请求超过当前适配器大小限制');
 return request;
}
