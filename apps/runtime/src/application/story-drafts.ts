...
async function writeCommand(store: StoryDraftStore, owner: InternalOwnerContext, commandId: string, action: string,
...
type Cursor = {version: 1; ownerId: string; datasetId: string; deleted: 'exclude' | 'only'; updatedAt: string; id: string};
function readCursor(raw: string, ownerId: string, datasetId: string, deleted: 'exclude' | 'only'): Cursor {
  try {
    if (!raw || raw.length > 2048 || !/^[a-zA-Z0-9_-]+$/.test(raw)) throw new Error();
    const value: unknown = JSON.parse(Buffer.from(raw, 'base64url').toString('utf8'));
    fields(value, ['version', 'ownerId', 'datasetId', 'deleted', 'updatedAt', 'id']);
    if (value.version !== 1 || value.ownerId !== ownerId || value.datasetId !== datasetId || value.deleted !== deleted || !isTimestamp(value.updatedAt)) throw new Error();
    return {version: 1, ownerId, datasetId, deleted, updatedAt: value.updatedAt, id: parseId(value.id)};
...
export async function listDrafts(store: StoryDraftStore, owner: InternalOwnerContext, input: DraftListInput = {}): Promise<DraftPage> {
  ...
  const datasetId = owner.datasetId;
  ...
  const cursor = input.cursor !== undefined ? readCursor(input.cursor, ownerId, owner.datasetId, deleted) : null;
  return store.read(ownerId, async scope => {
    ...
    const nextCursor = rows.length > limit && last ? Buffer.from(JSON.stringify({version: 1, ownerId, datasetId, deleted, updatedAt: last.updatedAt, id: last.id} satisfies Cursor)).toString('base64url') : null;
