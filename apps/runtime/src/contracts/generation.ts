...
export function parseOpeningQuote(value:unknown):OpeningQuoteInput{
 const protocol=parseProtocol(value);try{fields(value,['protocolVersion','datasetId','commandId','experienceId','expectedExperienceRevision','kind']);if(value.kind!=='opening')throw Error();
...
export function parseAcceptGeneration(value:unknown):AcceptGenerationInput{
 const protocol=parseProtocol(value);try{fields(value,['protocolVersion','datasetId','commandId','experienceId','expectedExperienceRevision','quoteId','consent']);if(value.consent!==true)throw Error();
