...
describe('internal story root CRUD on a real SQLite file', () => {
  ...
  it('soft-deletes, hides default reads, and restores with a new revision', async () => {
    ...
    const deleted = await deleteDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1});
    ...
    const restored = await restoreDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 2});
  ...
  it('isolates reads, lists and all write actions by active owner', async () => {
    const {data} = await createDraft(new PrismaStoryDraftStore(db), owner, command()); const other = {ownerId: v7()}; const now = new Date();
    ...
    await expect(updateDraft(new PrismaStoryDraftStore(db), other, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '越权'}})).rejects.toThrow('STORY_NOT_FOUND');
    await expect(deleteDraft(new PrismaStoryDraftStore(db), other, {commandId: v7(), id: data.id, expectedRevision: 1})).rejects.toThrow('STORY_NOT_FOUND');
    await expect(restoreDraft(new PrismaStoryDraftStore(db), other, {commandId: v7(), id: data.id, expectedRevision: 1})).rejects.toThrow('STORY_NOT_FOUND');
  ...
  it('rejects stale update/delete/restore without changing data or receipts', async () => {
    ...
    await updateDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '先保存'}});
    await expect(updateDraft(new PrismaStoryDraftStore(second), owner, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '旧窗口'}})).rejects.toThrow('REVISION_CONFLICT');
    await expect(deleteDraft(new PrismaStoryDraftStore(second), owner, {commandId: v7(), id: data.id, expectedRevision: 1})).rejects.toThrow('REVISION_CONFLICT');
    await deleteDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 2});
    await expect(restoreDraft(new PrismaStoryDraftStore(second), owner, {commandId: v7(), id: data.id, expectedRevision: 2})).rejects.toThrow('REVISION_CONFLICT');
  ...
  it('does not cascade deletion into frozen versions', async () => {
    ...
    await deleteDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1});
  ...
  it('uses the injected Clock/IdFactory and never moves updatedAt backwards', async () => {
    ...
    const updated = await updateDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '时钟倒退时也保留顺序'}}, backwards);
  ...
  it('rolls back an update if receipt insertion fails instead of leaving half a command', async () => {
    ...
    await expect(updateDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '不应留下'}}, services)).rejects.toMatchObject({code: 'P2002'});
  ...
  it('refuses unknown stored schema and does not silently downgrade its content', async () => {
    ...
    await expect(updateDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 1, patch: {title: '不要覆盖未来数据'}})).rejects.toThrow('STORED_STORY_INVALID');
  ...
  it('replays restore even after a later deletion without undoing the latest state', async () => {
    ...
    await deleteDraft(new PrismaStoryDraftStore(db), owner, {commandId: v7(), id: data.id, expectedRevision: 3});
  ...
  it('rejects a cursor from another owner even when both identities are active', async () => {
    ...
    const {nextCursor} = await listDrafts(new PrismaStoryDraftStore(db), owner, {limit: 1}); const now = new Date(), another = {ownerId: v7()};
...
describe('owner-scoped story store contract on the shared real SQLite fixture', () => {
  ...
  it('composes all six operations with the real adapter and injected runtime services', async () => {
    ...
    const changed = await service.update(owner, {commandId: v7(), id: created.data.id, expectedRevision: 1, patch: {title: '组合更新'}});
    expect(changed.data.revision).toBe(2);
    const deleted = await service.delete(owner, {commandId: v7(), id: created.data.id, expectedRevision: 2});
    ...
    const restored = await service.restore(owner, {commandId: v7(), id: created.data.id, expectedRevision: 3});
