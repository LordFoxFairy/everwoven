...
it('executes create/get/update/delete/restore through real authenticated HTTP and a new DB connection each time',async()=>{
 ...
 const changed=await mutation('update',{commandId:v7(),id:draft.id,expectedRevision:1,patch:{title:'已打磨',settings:updatedSettings}});expect(changed.status).toBe(200);
...
it('returns validation errors and permits explicit deleted-record reads without leaking internal errors',async()=>{
 ...
 expect((await mutation('create',{commandId:v7(),title:'旧格式',settings:legacy})).status).toBe(400);
