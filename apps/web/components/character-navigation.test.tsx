...
function setup(){const session={session:vi.fn().mockResolvedValue({authenticated:true,datasetId}),connect:vi.fn().mockResolvedValue(undefined),logout:vi.fn()};const client={list:vi.fn().mockResolvedValue({items:[],nextCursor:null,totalMatching:0}),get:vi.fn().mockResolvedValue(dto()),create:vi.fn().mockImplementation(async input=>({data:{...dto(input.name),settings:input.settings},replayed:false})),update:vi.fn(),delete:vi.fn(),restore:vi.fn()};vi.mocked(createAuthoringSessionClient).mockReturnValue(session);vi.mocked(createCharacterClient).mockReturnValue(client);return{session,client};}
...
it('enters original formal characters directly, connects above navigation, and never touches failing browser storage',async()=>{
 const {session,client}=setup();session.session.mockResolvedValueOnce({authenticated:false});const read=vi.spyOn(Storage.prototype,'getItem').mockImplementation(()=>{throw Error('no storage');}),write=vi.spyOn(Storage.prototype,'setItem').mockImplementation(()=>{throw Error('no storage');});
 render(<Platform environment="dev" databaseEnabled/>);click('角色库');await screen.findByLabelText('本机连接码');click('创建角色');change('角色姓名','未连接输入');
 change('本机连接码','code');click('连接本机');await screen.findByText('已连接本机');
...
it('keeps unknown mounted across sidebar/Create/onUse, reconnects same dataset and confirms the exact command',async()=>{
 ...
 click('确认上次角色命令');await screen.findByLabelText('本机连接码');change('角色姓名','后来输入');change('本机连接码','again');click('连接本机');await screen.findByText('已连接本机');
...
it('different-dataset reconnect retains original input and requires explicit new creation without retargeting the pending command',async()=>{
 ...
 click('确认上次角色命令');await screen.findByLabelText('本机连接码');session.session.mockResolvedValue({authenticated:true,datasetId:'01994b80-0000-7000-8000-000000000098'});change('本机连接码','new');click('连接本机');await screen.findByRole('button',{name:'从保留文本新建角色'});
...
it('keeps connection controls inside the shell content and the mounted Editor content',async()=>{
 const {session}=setup();session.session.mockResolvedValue({authenticated:false});render(<Platform environment="dev" databaseEnabled/>);
 const code=await screen.findByLabelText('本机连接码');expect(code.closest('main')?.id).toBe('main');click('创作一个剧本');
 expect(screen.getByLabelText('本机连接码').closest('main')?.className).toBe('editor-main');click('03角色配置');change('角色姓名 *','连接中保留');
 session.session.mockResolvedValue({authenticated:true,datasetId});change('本机连接码','code');click('连接本机');await screen.findByText('已连接本机');expect((screen.getByLabelText('角色姓名 *') as HTMLInputElement).value).toBe('连接中保留');
});
...
it('home reconnects A to B without ever editing characters and can navigate immediately',async()=>{
 const {session,client}=setup();client.list.mockRejectedValueOnce({status:401});render(<Platform environment="dev" databaseEnabled/>);await screen.findByLabelText('本机连接码');
 session.session.mockResolvedValue({authenticated:true,datasetId:'01994b80-0000-7000-8000-000000000098'});change('本机连接码','B');click('连接本机');await screen.findByText('已连接本机');click('角色库');
...
it('a retained character on home can reach only its recovery page after cross-dataset reconnect',async()=>{
 ...
 let reject!:(reason:unknown)=>void;client.list.mockReturnValueOnce(new Promise((_,no)=>{reject=no;}));click('刷新角色');click('我的世界');expect(screen.getByRole('heading',{name:'让想象发生。让故事，属于你。'})).toBeTruthy();await act(async()=>reject({status:401}));await screen.findByLabelText('本机连接码');
 session.session.mockResolvedValue({authenticated:true,datasetId:'01994b80-0000-7000-8000-000000000098'});change('本机连接码','B');click('连接本机');await screen.findByText('已连接本机');
