...
import {usePlayController} from '../lib/experience/use-play-controller';
import {readPlayRoute} from '../lib/experience/play-route';
import {ExperienceLibrary} from './experience-library';
...
function PlatformContent({databaseEnabled}:{databaseEnabled:boolean}){
 ...
 const {controller:playController,state:playState}=usePlayController();
 ...
 const restoredPlay=useRef(false);
 useEffect(()=>{if(demo||session.state.status!=='connected'||restoredPlay.current)return;restoredPlay.current=true;const route=readPlayRoute(window.location.hash);if(route)void playController.open(route.experienceId,route);},[demo,session.state.status,playController]);
 function enterPlay(id:string){openingController.close();setView('saves');void playController.open(id);}
 ...
 function edit(s?:Story){if(!mayLeave())return;openingController.close();setCharacterSource(undefined);setNotice('');if(!demo&&!storyController.newDraft())return;setDraft(s?structuredClone(s):blankStory());setView('editor');}
 async function openStory(id:string){if(!mayLeave())return;openingController.close();if(await storyController.open(id)){setCharacterSource(undefined);setNotice('');setView('editor');}}
 ...
 <footer className="page-footer"><span>未完 / 每个故事，都有你的痕迹。</span><span>{view==='library'&&databaseEnabled?'SQLite 本机保存 · 尚未配置备份':demo?'浏览器本地保存 · 不代表云端备份':'故事与游玩进展保存到本机'}</span></footer></AppShell>{!demo&&<StoryPreparation onStart={enterPlay} controller={openingController} state={openingState} unsavedChanges={false} triggerRef={preparationTrigger} connectionSlot={<LocalConnection/>} returnLabel="返回我的游玩"/>}{notice&&<div role="status" className="toast">{notice}<button aria-label="关闭提示" onClick={()=>setNotice('')}><X size={16}/></button></div>}{selected&&<StartDialog story={selected} onClose={()=>setSelected(null)} onStart={()=>start(selected)}/>}</>
}
