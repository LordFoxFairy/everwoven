...
export function Platform({environment='demo'}:{environment?:AppEnvironment}){return <AppEnvironmentContext.Provider value={environment}><PlatformContent key={environment}/></AppEnvironmentContext.Provider>;}
function PlatformContent(){
 ...
 function mayLeave(){return !characterPending.busy&&(!characterPending.dirty||confirm('角色有未保存的修改，确定离开吗？'));}
 ...
 <footer className="page-footer"><span>未完 / 每个故事，都有你的痕迹。</span><span>本地保存 · 不代表云端备份</span></footer></AppShell>{notice&&<div role="status" className="toast">{notice}<button aria-label="关闭提示" onClick={()=>setNotice('')}><X size={16}/></button></div>}{selected&&<StartDialog story={selected} onClose={()=>setSelected(null)} onStart={()=>start(selected)}/>}</>
}
