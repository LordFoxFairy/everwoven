...
export function Platform({environment='demo',databaseEnabled=false}:{environment?:AppEnvironment;databaseEnabled?:boolean}){return <AppEnvironmentContext.Provider value={environment}><PlatformContent key={environment} databaseEnabled={databaseEnabled}/></AppEnvironmentContext.Provider>;}
function PlatformContent({databaseEnabled}:{databaseEnabled:boolean}){
 ...
 useEffect(()=>{try{setData(readLibrary(environment));}catch{setError('本地数据读取失败。为保护原数据，暂时关闭保存；请检查浏览器存储。');}setReady(true);},[]);
 function persist(next:Library){if(error){setNotice(error);return false;}try{writeLibrary(next,environment);setData(next);return true;}catch(e){setNotice('保存失败：'+(e instanceof Error?e.message:'本地存储不可用')+' 输入仍保留。');return false;}}
 ...
 if(view==='editor')return <Editor initial={draft} characters={data.characters} onSave={saveDraft} onSaveCharacter={saveCharacter} onBack={()=>setView('library')} onPlay={s=>setSelected(structuredClone(s))} notice={notice} dialog={selected&&<StartDialog story={selected} onClose={()=>setSelected(null)} onStart={()=>start(selected)}/>}/>;
 ...
 {view==='characters'&&<CharacterLibrary characters={data.characters} onSave={saveCharacter} onPendingChange={setCharacterPending} onUse={c=>{setDraft(applyCharacterTemplate(blankStory(),c));setView('editor');}}/>}
 <footer className="page-footer"><span>未完 / 每个故事，都有你的痕迹。</span><span>{view==='library'&&databaseEnabled&&storageTab==='database'?'SQLite 本机保存 · 尚未配置备份':'浏览器本地保存 · 不代表云端备份'}</span></footer></AppShell>{notice&&<div role="status" className="toast">{notice}<button aria-label="关闭提示" onClick={()=>setNotice('')}><X size={16}/></button></div>}{selected&&<StartDialog story={selected} onClose={()=>setSelected(null)} onStart={()=>start(selected)}/>}</>
}
