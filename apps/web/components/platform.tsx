...
function PlatformContent({databaseEnabled}:{databaseEnabled:boolean}){
 ...
 {view==='saves'&&!demo&&<ExperienceLibrary controller={openingController} state={openingState} datasetId={session.state.datasetId} connectionEpoch={session.invalidate} onCreate={()=>edit()} onOpen={(id,trigger)=>{preparationTrigger.current=trigger;void openingController.openExisting(id);}}/>}
