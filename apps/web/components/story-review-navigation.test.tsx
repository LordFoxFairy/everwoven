...
async function setup(){
 const client=storyClient(),session={session:vi.fn().mockResolvedValue({authenticated:true,datasetId}),connect:vi.fn().mockResolvedValue(undefined),logout:vi.fn().mockResolvedValue(undefined)};
...
async function reconnect(session:Awaited<ReturnType<typeof setup>>['session']){
 session.session.mockResolvedValue({authenticated:true,datasetId:otherDataset});fireEvent.change(screen.getByLabelText('本机连接码'),{target:{value:'fixture-only'}});fireEvent.click(screen.getByRole('button',{name:'连接本机'}));await screen.findByText('已连接本机');
}
it.each(['delete','restore'] as const)('completed list %s does not strand home after 401 and a dataset switch',async kind=>{
 ...
 client.list.mockRejectedValueOnce(new StoryClientError('LOCAL_SESSION_INVALID',401,'rejected'));fireEvent.click(screen.getByRole('button',{name:'刷新剧本'}));await screen.findByLabelText('本机连接码');
...
it('a real unknown list command still blocks leaving and only explicit recovery can create in a new dataset',async()=>{
 ...
 client.list.mockRejectedValueOnce(new StoryClientError('LOCAL_SESSION_INVALID',401,'rejected'));fireEvent.click(screen.getByRole('button',{name:'刷新剧本'}));await screen.findByLabelText('本机连接码');await reconnect(session);
