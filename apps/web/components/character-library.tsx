...
import {characterSelection,type CharacterSelection,type PendingState} from '../lib/authoring/character-viewmodel';
import styles from './character-library.module.css';

type Props={onUse:(c:CharacterSelection)=>void;onPendingChange:(state:PendingState)=>void}&({controller:CharacterController;characters?:never;onSave?:never}|{controller?:never;characters:Character[];onSave:(c:Character)=>Promise<CharacterSelection>});
...
function Cards({characters,formal,busy,edit,use}:{characters:CharacterSelection[];formal:boolean;busy:boolean;edit:(c:CharacterSelection)=>void;use:(c:CharacterSelection)=>void}){
 ...
 {formal?<div className={styles.portrait} aria-label="正式角色头像待接入">{[...c.name][0]}<small>{c.source?.portraitAssetId?'已保留正式头像引用；读取待接入':'正式头像上传待接入'}</small></div>:<CharacterPortrait character={c}/>}
...
function FormalLibrary({controller,onUse,onPendingChange}:Props&{controller:CharacterController}){
 ...
 useProtection({dirty:s.dirty,busy:s.saving||s.reading,...(s.unknown||s.datasetChanged?{unknown:true}:{})},onPendingChange);
 function mayDiscard(){const current=controller.getSnapshot();if(current.saving||current.reading)return false;if(current.unknown||current.datasetChanged)return false;return !current.dirty||confirm('放弃当前未保存的角色修改？');}
 ...
 return <><Heading begin={begin} busy={s.busy}/><p className={styles.hint}>正式角色只保存到本机 SQLite，不读取浏览器角色或图片。姓名是唯一必填项。</p>
 {s.datasetChanged&&<div className={styles.warning} role="alert"><p>数据已重置，原命令停止重放；工作文本仍保留。</p><button className="secondary" disabled={s.busy||!s.connected} onClick={()=>controller.fromRetained()}>从保留文本新建角色</button></div>}
 ...
 {s.items.length>0&&<Cards characters={cards} formal busy={s.busy||s.unknown||s.datasetChanged} edit={c=>read(c.id)} use={c=>{if(mayDiscard())onUse(c);}}/>}
 ...
 {s.editing&&<form aria-label="角色编辑" className={styles.form} onSubmit={e=>{e.preventDefault();attempt(controller.save());}}>
 <header><div><small>{s.confirmed?'编辑角色模板':'新的相遇'}</small><h2>{s.fields.name||'定义 TA 的模样'}</h2></div><button type="button" className="text-button" disabled={s.busy||s.unknown||s.datasetChanged} onClick={()=>{if(mayDiscard())controller.close();}}>收起</button></header>
 <Fields draft={s.fields} field={(key,value)=>controller.field(key,value)} disabled={s.reading||Boolean(s.confirmed?.deletedAt)}/>
 <p className={styles.hint}>{s.fields.portraitAssetId?'已有正式头像引用保持不变。':''}正式图片上传和读取待接入，不使用浏览器图片。</p>
 <p role="status">{s.saving?'正在保存提交的版本…':s.dirty?'有未保存修改':s.confirmed?`已确认 · 修订 ${s.confirmed.revision}`:'尚未创建'}</p>
 {!s.datasetChanged&&(s.unknown?<button type="button" className="primary" disabled={s.busy||!s.connected} onClick={()=>attempt(controller.confirm())}>确认上次角色命令</button>:s.confirmed?.deletedAt?<button type="button" className="primary" disabled={s.busy||!s.connected} onClick={()=>attempt(controller.lifecycle('restore'))}>恢复角色</button>:<button type="submit" className="primary" disabled={s.busy||!s.connected}><Check size={16}/>保存角色模板</button>)}
 {s.confirmed&&!s.unknown&&!s.datasetChanged&&<><button type="button" className="text-button" disabled={s.busy||!s.connected} onClick={()=>read(s.confirmed!.id)}>重新载入角色</button>{!s.confirmed.deletedAt&&<button type="button" className="text-button" disabled={s.busy||!s.connected} onClick={()=>{if(confirm('删除这个角色模板？已有剧本与固定版本不改变。'))attempt(controller.lifecycle('delete'));}}>删除角色</button>}</>}
 {s.message&&<p role="status" className={styles.hint}>{s.message}</p>}
 </form>}</div>
 {s.error&&!s.datasetChanged&&<div role="alert">{s.error}{readId&&!s.unknown&&<button disabled={s.busy||!s.connected} onClick={()=>read(readId)}>重试读取</button>}</div>}
 {s.unknown&&!s.datasetChanged&&<p role="status">上次角色命令结果待确认；仅确认原提交，不自动重发当前输入。</p>}
 </>;
}
function DemoLibrary({characters,onSave,onUse,onPendingChange}:{characters:Character[];onSave:(c:Character)=>Promise<CharacterSelection>;onUse:(c:CharacterSelection)=>void;onPendingChange:(s:PendingState)=>void}){
 ...
 const busy=imageBusy||saveBusy,dirty=!!draft&&JSON.stringify(draft)!==saved;useProtection({dirty,busy},onPendingChange);
 function mayDiscard(){return !busy&&!lock.current&&(!dirty||confirm('放弃当前未保存的角色修改？'));}
 function begin(c?:Character){if(!mayDiscard())return;const next=c?structuredClone(c):blank();setDraft(next);setSaved(JSON.stringify(next));setMessage('');}
 async function save(){if(!draft||lock.current||imageBusy)return;const next=structuredClone(draft);
  ...
  lock.current=true;setSaveBusy(true);setMessage('');try{const confirmed=await onSave(next);if(alive.current){setSaved(JSON.stringify(confirmed));setMessage('角色模板已保存到当前浏览器；后续修改仍需保存。');}}
 ...
 {characters.length?<Cards characters={characters} formal={false} busy={busy} edit={begin} use={c=>{if(mayDiscard())onUse(c);}}/>:<div className="empty"><h3>先认识一个人</h3><p>姓名足以保存草稿，设定与图片可以慢慢补充。</p>{!draft&&<button className="secondary" onClick={()=>begin()}>创建第一个角色</button>}</div>}
 ...
 <ImageAssetPicker role="character" assetId={draft.imageAssetId} onChange={id=>setDraft(prev=>prev?{...prev,imageAssetId:id}:prev)} onBusyChange={setImageBusy}/>
