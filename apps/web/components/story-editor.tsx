...
import {ImageAssetPicker,useStoryArtwork} from './story-assets';
...
export function Editor({initial,characters,onSave,onSaveCharacter,onBack,onPlay,notice,dialog,formal=false,characterSource,characterPending,characterReset=false,characterEpoch,onResetCharacter,connectionSlot}:{initial:Story;characters:Character[];onSave:(s:Story)=>boolean;onSaveCharacter:(c:CharacterSelection)=>Promise<CharacterSelection>;formal?:boolean;characterSource?:CharacterSource;characterPending?:PendingState;characterReset?:boolean;characterEpoch?:unknown;connectionSlot?:React.ReactNode;onResetCharacter?:()=>void;onBack:()=>void;onPlay:(s:Story)=>void;notice:string;dialog:React.ReactNode}){
 ...
 const busy=Object.values(busyRoles).some(Boolean)||characterSaving||Boolean(characterPending?.busy),dirty=JSON.stringify(s)!==saved,art=useStoryArtwork(formal?{...s,assets:undefined,artId:undefined}:s);
 useEffect(()=>{const handler=(e:BeforeUnloadEvent)=>{if(dirty||busy||characterPending?.unknown){e.preventDefault();e.returnValue='';}};window.addEventListener('beforeunload',handler);return()=>window.removeEventListener('beforeunload',handler);},[dirty,busy,characterPending?.unknown]);
 ...
 function characterCopy(story:Story):CharacterSelection{return {id:createEntityId(),name:story.character,personality:story.personality,appearance:story.appearance??'',speakingStyle:story.speakingStyle??'',boundaries:story.boundaries??'',...(formal?{source:characterSource}:{imageAssetId:story.assets?.character})};}
 function contentKey(c:CharacterSelection){const {id,source,...fields}=c;return JSON.stringify(fields);}
 async function saveCharacter(){
  if(characterLock.current||busy||characterReset)return;
 ...
 return <div className="editor-shell">
  <header className="editor-header"><button disabled={busy} className="text-button" onClick={()=>{if(characterPending?.unknown){setFeedback('上次角色命令结果待确认，请先确认原命令。');return;}if(!dirty||confirm('还有未保存的修改，确定离开吗？'))onBack();}}><ArrowLeft size={18}/>我的剧本</button><span className="editor-title">{s.title||'未命名剧本'}<small>{busy?'正在处理，请稍候…':dirty?'有未保存修改':formal?'未保存的编辑副本':'浏览器草稿'}</small></span><div><button disabled={busy||formal} className="secondary" onClick={save}><SaveIcon size={16}/>保存草稿</button><button disabled={busy||formal} className="primary" onClick={()=>{if(save())onPlay(s);}}><Play size={16}/>保存并进入准备</button></div></header>
  ...
  {section===2&&<>
   ...
   {!formal&&assetPicker('character')}{formal&&<p className="inline-note">{characterSource?.portraitAssetId?'已有正式头像引用保留；':''}正式头像上传与读取待接入，不使用浏览器图片。</p>}{s.artId&&!s.assets?.character&&<div className="inline-note"><p>此剧本带有演示角色视觉。选择自己的图片会替换它。</p><button type="button" className="text-button" onClick={()=>setS(prev=>({...prev,artId:undefined}))}>移除演示视觉</button></div>}
   <button type="button" className="secondary" disabled={busy||characterReset} onClick={()=>void saveCharacter()}><Users size={16}/>{characterPending?.unknown?'确认上次角色命令':'另存为角色模板'}</button><div className="inline-note"><Users size={20}/><p>一期支持一位主要角色。角色模板、剧本草稿与游玩设定独立，修改不会串到其他故事。</p></div>
  </>}
  {section===3&&formal&&<p className="inline-note">正式素材服务与剧本聚合待接入。当前字段仅为编辑副本，不写浏览器素材库。</p>}{section===3&&!formal&&<>{assetPicker('cover')}{assetPicker('opening')}<label>未选自定义图片时的封面</label><div className="image-options">{Object.entries(images).map(([key,url],i)=><button aria-pressed={s.image===key} className={s.image===key?'image-option picked':'image-option'} key={key} onClick={()=>field('image',key)}><img src={url} alt={['海边','夜空','森林','小屋'][i]}/><span>{['海边','夜空','森林','小屋'][i]}{s.image===key&&<Check size={14}/>}</span></button>)}</div><div className="inline-note"><p>图片现阶段只保存在本机，不代表已经交给视频模型。正式生成会按供应商与模型校验素材用途；封面不进入生成提示。</p></div></>}
  </div><>{formal&&<p className="inline-note">当前为剧本编辑副本，正式聚合尚未保存；带入的角色来源不是已冻结版本。</p>}{characterReset&&<div role="alert">数据已重置，工作文本仍保留。<button type="button" disabled={busy} onClick={onResetCharacter}>从保留文本新建角色</button></div>}</><div className="editor-bottom"><span>{section+1} / 4</span>{section<3?<button disabled={busy} className="primary" onClick={()=>setSection(section+1)}>继续配置 →</button>:<button disabled={busy||formal} className="primary" onClick={save}>保存剧本<Check size={16}/></button>}</div>{feedback&&<p className="inline-note" role="status">{feedback}</p>}{notice&&<p className="inline-note" role="status">{notice}</p>}</main>
