...
export function Editor({initial,characters,onSave,onSaveCharacter,onBack,onPlay,notice,dialog,formal=false,characterSource,characterPending,characterReset=false,characterEpoch,onResetCharacter,connectionSlot}:{initial:Story;characters:Character[];onSave:(s:Story)=>boolean;onSaveCharacter:(c:CharacterSelection)=>Promise<CharacterSelection>;formal?:boolean;characterSource?:CharacterSource;characterPending?:PendingState;characterReset?:boolean;characterEpoch?:unknown;connectionSlot?:React.ReactNode;onResetCharacter?:()=>void;onBack:()=>void;onPlay:(s:Story)=>void;notice:string;dialog:React.ReactNode}){
 const [s,setS]=useState(initial),[section,setSection]=useState(0),[saved,setSaved]=useState(JSON.stringify(initial));
 const [editingKey]=useState(createEntityId),binding=useAssetBinding();
 ...
 const recovery=characterReset||imageReset;
 ...
 function recover(){if(Object.values(uploads).every(x=>x.controller.discardForDatasetChange())){setPortrait(null);onResetCharacter?.();}}
 ...
 useEffect(()=>{attempt.current++;characterLock.current=false;setCharacterSaving(false);},[characterEpoch]);
 const busy=Object.values(busyRoles).some(Boolean)||Object.values(uploads).some(x=>x.state.busy)||characterSaving||Boolean(characterPending?.busy),dirty=JSON.stringify(s)!==saved||JSON.stringify(portrait)!==portraitBaseline.current,art=useStoryArtwork(formal?{...s,assets:undefined,artId:undefined}:s);
 useEffect(()=>{const handler=(e:BeforeUnloadEvent)=>{if(dirty||busy||characterPending?.unknown||imageUnknown||recovery){e.preventDefault();e.returnValue='';}};window.addEventListener('beforeunload',handler);return()=>window.removeEventListener('beforeunload',handler);},[dirty,busy,characterPending?.unknown,imageUnknown,recovery]);
 function field(key:keyof Story,value:string){setS(prev=>({...prev,[key]:value}));}
 function save(){if(formal){setFeedback('正式剧本聚合尚未接入；当前输入仅保留在编辑器，没有保存到浏览器或冻结角色版本。');return false;}const ok=onSave(s);if(ok)setSaved(JSON.stringify(s));return ok;}
 function assetPicker(role:AssetRole){return <ImageAssetPicker role={role} assetRef={formal?portrait:s.assets?.[role]?{kind:'demo',id:s.assets[role]!}:null} upload={uploads[role]} onChange={ref=>selectImage(role,ref)} disabled={characterSaving||Boolean(characterPending?.busy)||Boolean(characterPending?.unknown)||recovery} onBusyChange={value=>setBusyRoles(prev=>prev[role]===value?prev:{...prev,[role]:value})}/>;}
 function characterCopy(story:Story):CharacterSelection{return {id:createEntityId(),name:story.character,personality:story.personality,appearance:story.appearance??'',speakingStyle:story.speakingStyle??'',boundaries:story.boundaries??'',portraitRef:formal?portrait:story.assets?.character?{kind:'demo',id:story.assets.character}:null,...(formal?{source:characterSource}:{imageAssetId:story.assets?.character})};}
 ...
 return <div className="editor-shell">
  <header className="editor-header"><button disabled={busy} className="text-button" onClick={()=>{if(characterPending?.unknown||imageUnknown||recovery||imageBlocked()){setFeedback('角色或图片命令结果待确认，请先处理原命令。');return;}if(!dirty||confirm('还有未保存的修改，确定离开吗？'))onBack();}}><ArrowLeft size={18}/>我的剧本</button><span className="editor-title">{s.title||'未命名剧本'}<small>{busy?'正在处理，请稍候…':dirty?'有未保存修改':formal?'未保存的编辑副本':'浏览器草稿'}</small></span><div><button disabled={busy||formal} className="secondary" onClick={save}><SaveIcon size={16}/>保存草稿</button><button disabled={busy||formal} className="primary" onClick={()=>{if(save())onPlay(s);}}><Play size={16}/>保存并进入准备</button></div></header>
  ...
  {section===0&&<><label>剧本名称 <span>*</span><input value={s.title} onChange={e=>field('title',e.target.value)} placeholder="为你的故事起一个名字" maxLength={60}/><small>一个令人想走进去的名字。</small></label><label>故事题材<select value={s.genre} onChange={e=>field('genre',e.target.value)}>{['浪漫','都市','奇幻','日常'].map(x=><option key={x}>{x}</option>)}</select></label><div className="inline-note"><BookOpen size={20}/><p>这是你的剧本，不是固定的剧情树。用户进入后可以用自己的对话和行动推动发展。</p></div></>}
  {section===1&&<><label>世界背景 <span>*</span><textarea value={s.world} onChange={e=>field('world',e.target.value)} placeholder="故事发生在哪里？这个世界有哪些规则？" rows={5}/></label><label>开局情境 <span>*</span><textarea value={s.opening} onChange={e=>field('opening',e.target.value)} placeholder="用几句话描述故事开始的这一刻…" rows={4}/><small>避免替玩家规定台词、行动和情感选择。</small></label></>}
  {section===2&&<>
   {characters.length>0&&<label>从角色库复制<select value="" disabled={busy} onChange={e=>{const c=characters.find(c=>c.id===e.target.value);if(c&&(!(s.character||s.personality||s.appearance||s.speakingStyle||s.boundaries||s.assets?.character||s.artId)||confirm('替换当前角色的姓名、设定与参考图？剧本背景和初始关系保持不变。')))setS(prev=>applyCharacterTemplate(prev,c));}}><option value="" disabled>选择角色模板</option>{characters.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select><small>完整复制外貌、表达习惯、边界与图片，后续独立编辑。</small></label>}
   ...
   {assetPicker('character')}{s.artId&&!s.assets?.character&&<div className="inline-note"><p>此剧本带有演示角色视觉。选择自己的图片会替换它。</p><button type="button" className="text-button" onClick={()=>setS(prev=>({...prev,artId:undefined}))}>移除演示视觉</button></div>}
  ...
  {section===3&&formal&&<p className="inline-note">正式封面与开场图保存仍待剧本聚合接入；本片只启用角色参考图，不写浏览器素材库。</p>}{section===3&&!formal&&<>{assetPicker('cover')}{assetPicker('opening')}<label>未选自定义图片时的封面</label><div className="image-options">{Object.entries(images).map(([key,url],i)=><button aria-pressed={s.image===key} className={s.image===key?'image-option picked':'image-option'} key={key} onClick={()=>field('image',key)}><img src={url} alt={['海边','夜空','森林','小屋'][i]}/><span>{['海边','夜空','森林','小屋'][i]}{s.image===key&&<Check size={14}/>}</span></button>)}</div><div className="inline-note"><p>图片现阶段只保存在本机，不代表已经交给视频模型。正式生成会按供应商与模型校验素材用途；封面不进入生成提示。</p></div></>}
  </div><>{formal&&<p className="inline-note">当前为剧本编辑副本，正式聚合尚未保存；带入的角色来源不是已冻结版本。</p>}{recovery&&<div role="alert">数据已重置，工作文本仍保留。<button type="button" disabled={busy} onClick={recover}>从保留文本新建角色</button></div>}</><div className="editor-bottom"><span>{section+1} / 4</span>{section<3?<button disabled={characterSaving||Boolean(characterPending?.busy)} className="primary" onClick={()=>setSection(section+1)}>继续配置 →</button>:<button disabled={busy||formal} className="primary" onClick={save}>保存剧本<Check size={16}/></button>}</div>{feedback&&<p className="inline-note" role="status">{feedback}</p>}{notice&&<p className="inline-note" role="status">{notice}</p>}</main>
