...
function client(){return {session:vi.fn<()=>Promise<AuthoringSession>>().mockResolvedValue({authenticated:true,datasetId}),connect:vi.fn().mockResolvedValue(undefined),logout:vi.fn().mockResolvedValue(undefined)};}
...
it('keeps editor mounted through reconnect and obtains dataset from verified session',async()=>{
 const c=client();c.session.mockResolvedValueOnce({authenticated:false});
 ...
 expect(c.connect).toHaveBeenCalledExactlyOnceWith('code');expect((screen.getByLabelText('draft') as HTMLInputElement).value).toBe('my text');
});
...
it('locks same-frame connection attempts and never leaks raw transport errors',async()=>{
 const c=client();c.session.mockResolvedValueOnce({authenticated:false});c.connect.mockRejectedValue(new Error('/private TOKEN'));
 ...
 await waitFor(()=>expect(c.connect).toHaveBeenCalledTimes(1));
...
it('ignores stale connect refresh and invalidate handles after a mode/client switch',async()=>{
 ...
 await stale.connect('old code');await stale.refresh();stale.invalidate();
...
it('ignores a prior authenticated epoch invalidation after reconnecting with the same client',async()=>{
 ...
 await act(async()=>{await captured.connect('renewed');});
 expect(c.connect).toHaveBeenCalledExactlyOnceWith('renewed');
