...
export function newSave(story:Story):Save{return {id:crypto.randomUUID(),story:structuredClone(story),createdAt:new Date().toISOString(),turns:[]};}
export function appendTurn(save:Save,text:string):Save{if(!text.trim())return save;return {...save,turns:[...save.turns,{id:crypto.randomUUID(),text:text.trim(),status:'recorded-demo',createdAt:new Date().toISOString()}]};}

export function pinMemory(save:Save,text:string):Save{if(!text.trim())return save;return {...save,memories:[...(save.memories??[]),{id:crypto.randomUUID(),text:text.trim(),createdAt:new Date().toISOString()}]};}
